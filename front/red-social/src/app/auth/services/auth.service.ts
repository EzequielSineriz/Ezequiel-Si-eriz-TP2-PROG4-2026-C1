...
export class AuthService {
  ...
  private mostrarAlertaGotica(titulo: string, texto: string, icono: 'success' | 'error') {
    return Swal.fire({
  ...
  login(credenciales: ILogin): void {
    this.http.post<IAuthResponse>(`${this.apiUrl}/auth/ingresar`, credenciales).subscribe({
      next: (response) => {
        console.log('Ingreso concedido:', response);

        // Guardamos el token de forma segura en el navegador para los Guards
        localStorage.setItem('paranormal_token', response.token);
        localStorage.setItem('paranormal_user', JSON.stringify(response.user));

        // Actualizamos el Signal reactivo
        this.usuarioActual.set(response.user);
      ...
      error: (err) => {
        ...
        this.mostrarAlertaGotica(
          'Ingreso Denegado',
          'Tus credenciales no coinciden con nuestros registros ocultos. Verificá tu usuario o clave.',
          'error'
        );
      }
    });
  ...
  return this.http.get<{ exists: boolean }>(`${this.apiUrl}/auth/check-email?email=${email}`).pipe(
...
iniciarContadorMaldito() {
    ...
    const tiempoEsperaPrueba = 10 * 1000;
  ...
  private mostrarModalExtensionSesion() {
    Swal.fire({
      title: '<span class="font-logo uppercase tracking-widest text-lg text-amber-500 drop-shadow-[0_0_8px_rgba(245,158,11,0.3)]">⚠️ Alianza por Expirar</span>',
      html: `
  ...
  refrescarTokenEspectral(): Observable<any> {
    ...
    return this.http.post<any>(`${this.apiUrl}/auth/refrescar`, {}, {
...
actualizarPerfilUsuario(id: string, descripcion: string, avatarArchivo: File | null): Observable<any> {
  ...
  return this.http.put(`http://localhost:3000/auth/usuarios/${id}/perfil`, formData, { headers }).pipe(
